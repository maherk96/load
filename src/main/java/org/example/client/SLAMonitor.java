    ...
    public SLAViolation checkSLA() {
        ...
        if (sla.getErrorRatePct() > 0) {
            ...
            if (currentErrorRate > sla.getErrorRatePct()) {
                return new SLAViolation("Error rate " + currentErrorRate + "% exceeds limit " + sla.getErrorRatePct() + "%");
            }
        }
        ...
        if (sla.getP95LtMs() > 0) {
            ...
            if (currentP95 > sla.getP95LtMs()) {
                return new SLAViolation("P95 response time " + currentP95 + "ms exceeds limit " + sla.getP95LtMs() + "ms");
            }
        }
