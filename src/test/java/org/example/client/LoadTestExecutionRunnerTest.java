...
/**
 * Unit tests for LoadTestExecutionRunner using real HTTP endpoints These tests focus on behavioral
 * verification rather than mock-based testing
 */
class LoadTestExecutionRunnerTest {
  ...
  void applyThinkTime_Fixed_ShouldDelayCorrectly() throws Exception {
    ...
    TestPlanSpec spec =
        createClosedWorkloadSpecWithThinkTime(
            1, 2, "10s", "0s", "0s", TestPlanSpec.ThinkTimeType.FIXED, 1000, 1000);
    LoadTestExecutionRunner runner = new LoadTestExecutionRunner(spec);
  ...
  void applyThinkTime_Random_ShouldDelayWithinRange() throws Exception {
    ...
    TestPlanSpec spec =
        createClosedWorkloadSpecWithThinkTime(
            1, 3, "10s", "0s", "0s", TestPlanSpec.ThinkTimeType.RANDOM, 500, 1000);
    LoadTestExecutionRunner runner = new LoadTestExecutionRunner(spec);
  ...
  void slaConfiguration_ShouldNotPreventExecution() throws Exception {
    // Given
    TestPlanSpec spec =
        createSpecWithSLA(1, 5, "5s", 50.0, 5000, TestPlanSpec.OnErrorAction.CONTINUE);
    LoadTestExecutionRunner runner = new LoadTestExecutionRunner(spec);
  ...
  void executeRequest_WithInvalidUrl_ShouldRecordError() throws Exception {
    ...
    spec.getTestSpec()
        .getGlobalConfig()
        .getTimeouts()
        .setConnectionTimeoutMs(2000); // Short timeout
    LoadTestExecutionRunner runner = new LoadTestExecutionRunner(spec);
    ...
    assertEquals(
        "ALL_ITERATIONS_COMPLETED", report.getTerminationReason()); // Should complete all attempts
  }
  ...
  private TestPlanSpec createSpecWithSLA(
      int users,
      int iterations,
      String holdFor,
      double errorRatePct,
